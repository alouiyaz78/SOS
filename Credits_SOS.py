...
from reportlab.platypus import SimpleDocTemplate, Table, TableStyle,Paragraph,Spacer,PageTemplate,Preformatted,PageBreak
...
from arabic_reshaper import arabic_reshapertimedelta
import ctypes
import docx2pdf
import subprocess
...
class Application(tbk.Window):
    def __init__(self):
       ...
       # self.credits_section = CreditsSection(self.paned_window, self, self.clients_section, self.impression_section)
        ...
        self.payement_section = PayementSection(self.paned_window, self, self.clients_section, self.credits_section,self.impression_section)
...
class IMFSection(tbk.Frame):
    ...
    def create_widgets(self):
        ...
        # Labels
        labels_imf_arab = ['الإسم الإجتماعي', 'العنوان',' الهاتف', 'البنك', 'رقم الحساب', 'الضمان الإجتماعي', 'السجل التجاري', 'العنوان الإلكتروني']
        ...
        tbk.Entry(frame_imf, textvariable=self.autre_adresse_imf, width=40).grid(column=2, row=1, padx=80, pady=20, columnspan=2,sticky='w')
        tbk.Entry(frame_imf, textvariable=self.autre_banque_imf, width=40).grid(column=2, row=3, padx=80, pady=20, columnspan=2,sticky='w')
        tbk.Entry(frame_imf, textvariable=self.autre_rib_imf, width=40).grid(column=2, row=4, padx=80, pady=20, columnspan=2,sticky='w')
    ...
    def insert_imf(self, raison_sociale_imf='', adresse_imf='', autre_adresse_imf='', tel_imf='', banque_imf='', autre_banque_imf='',
...
class CreditsSection(tbk.Frame):
    ...
    def create_widgets(self,amortization_data=None):
        ...
        self.recherche_combo=tbk.Combobox(frame_credits,textvariable=self.clients_var, state="readonly", bootstyle="success",
                                        width=30)
        ...
        labels_credits = [ 'Nom', 'Prenom', 'CIN', 'Date_Credit', 'Montant (DT)','Durée (en mois)', 'Duré de grace','Crédit ID', 'Taux d"interet','','Mensualités' ]
        ...
        entries_credits = [
            ...
            tbk.Scale(frame_credits, variable=self.taux_interet_var,bootstyle="warning",length=200, from_=0, to=30,orient=tk.HORIZONTAL,
        ...
        btn_valider = tbk.Button(frame_credits, text="Valider", style="success.TButton",
                   command=lambda: self.valider_credit(  self.Nom_var_credit.get(), self.Prenom_var_credit.get(),
                       self.cin_var_credit.get(),self.Date_Credit.entry.get(),montant_var.get(),duree_dropdown.get(),amortization_data))


        btn_imprimer = tbk.Button(frame_credits, text="Imprimer Table", style="success.TButton",command=lambda: self.impression_section.generate_and_print_table(self.Nom_var_credit.get(), self.Prenom_var_credit.get(),
                       self.cin_var_credit.get()))
        btn_contrat=tbk.Button(frame_credits,text="Imprimer contrat",style="Success",width=30,command=lambda:self.impression_section.generate_and_print_contrat(self.Nom_var_credit.get(),self.Prenom_var_credit.get(), self.cin_var_credit.get()))
        btn_traite = tbk.Button(frame_credits, text="Imprimer traites", style="danger",width=40,command=lambda:self.impression_section.generate_and_print_traites(self.Nom_var_credit.get(),self.Prenom_var_credit.get(), self.cin_var_credit.get()))
...
class PayementSection(tbk.Frame):
    ...
    def create_widgets(self):
        ...
        style.configure("mystyle.Treeview", highlightthickness=1, bd=0,
                        font=('Calibri', 11))  # Modify the font of the body
        style.configure("mystyle.Treeview.Heading", font=('Calibri', 12, 'bold'),background="#808080")  # Modify the font of the headings
       ...
       # style.layout("mystyle.Treeview", [('mystyle.Treeview.treearea', {'sticky': 'nswe'})])  # Remove the borders
        ...
        self.entry_reste = tbk.Entry(frame_credit, font=('Halvetica', 12), foreground='white',width=20, style='warning')
        ...
        self.valider=tbk.Button(frame_credit, width=20, style='success',text='Valider',command=self.effectuer_paiement)
        ...
        self.taux_recouv=Meter(label_stat_client,bootstyle="success",subtext='Taux Rec',textright="%",subtextstyle="light",metersize=150,
        stripethickness=10,textfont=('Halvetica', 10,'bold'),amountused=self.taux_recouvrement,meterthickness=10,padding=50)
        ...
        self.date_np_entry = tbk.Entry(self.titre_2, font=('Helvetica', 10), width=15, style="danger", foreground='white')
    ...
    def update_amortization_table(self, event=None):
        ...
        c.execute("SELECT echeance_date,echeance_numero,Montant_echeance,Reste_du_credit,Interet,paye,date_paye FROM amortissement WHERE credit_id = ?", (selected_credit_id,))
    ...
    def valider_montant_paiement(self, montant_paiement, reste_credit):
        ...
        if not (montant_paiement > 0 and isinstance(montant_paiement, float)) :
            messagebox.showerror("Erreur de montant","Le montant de paiement doit être un float positif avec 3 chiffres après la virgule.")
            return False
...
class ImpressionManager(tbk.Frame):
            ...
            if result_1:
                ...
                if selected_credit_id:
                    ...
                    if user_choice == 'yes':
                        ...
                        style = ParagraphStyle(name='CenteredStyle', alignment=1, fontSize=12,fontName="SimplifiedArabic")
                        ...
                        style = ParagraphStyle(name='RTLStyle', alignment=TA_RIGHT,fontSize=12,fontName="SimplifiedArabic")
            ...
            if result_1:
                ...
                if selected_credit_id:
                    ...
                    for page_num in range(pages_needed):
                        ...
                        for traite_num, (
                        echeance_date, echeance_numero, montant_echeance, interet, reste_du_credit) in enumerate(
    ...
    def remplir_contrat(self, can, page, nom_client, prenom_client, cin_client, date_delivrance, adresse, delegation,
        ...
        listes = [str(nom_client), str(prenom_client), str(adresse), str(delegation), str(secteur_activite),str(sous_secteur), str(raison_sociale),
                  str(adresse_imf)]
        ...
        try:
            ...
            if result_client:
                ...
                if selected_credit_id:
                    ...
                    temp_filename = "C:/Users/User/PycharmProjects/ligne_credits/temp_contract.pdf"
    ...
    def generer_rapport(self, popup, debut, fin):
        ...
        titre_label = Label(rapport_window, text=f"Recouvrement journalier de {debut} à {fin}",font=('Calibri', 14, 'bold'))
        ...
        Button(bottom_frame, text="Imprimer", command=lambda: self.imprimer_rapport(tree)).pack(pady=10)
